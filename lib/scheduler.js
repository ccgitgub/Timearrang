 ...
 */
function generateAssignments(teachers, timeSlots, options = {}) {
  const classTeacherMap = options.classTeacherMap || new Map(); // Map<duty_name, teacher_id>

  // Pre-populate classTeacherUsed from locked seats so hard-locked slots don't trigger a
  // second soft-preferred assignment in an earlier slot for the same duty.
  const classTeacherUsed = new Set();
  for (const slot of timeSlots) {
    for (const duty of slot.duties) {
      if (!duty.lockedSeats) continue;
      for (const teacherId of duty.lockedSeats.values()) {
        if (teacherId != null && classTeacherMap.get(duty.name) === teacherId) {
          classTeacherUsed.add(duty.name);
        }
      }
    }
  }
  ...
  for (let i = 0; i < timeSlots.length; i++) {
    ...
    for (const seat of seats) {
      ...
      // 軟性優先：各班班主任至少負責該班一節
      if (!classTeacherUsed.has(seat.duty_name) && classTeacherMap.has(seat.duty_name)) {
        const ctId = classTeacherMap.get(seat.duty_name);
        const ctTeacher = pool.find((t) => t.id === ctId);
        if (ctTeacher) {
          classTeacherUsed.add(seat.duty_name);
          updateGrade(ctTeacher.id, seat.duty_name, duration);
          busyInSlot.add(ctTeacher.id);
          recordAssignment(slot.label, seat.duty_name, ctTeacher.id, seat.slot_index);
          results.push({ duty_id: seat.duty_id, slot_index: seat.slot_index, teacher_id: ctTeacher.id });
          continue;
        }
      }
