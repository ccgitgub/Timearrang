...
function cell(lines, { header = false, align = AlignmentType.CENTER, width, shading } = {}) {
  const values = Array.isArray(lines) ? lines : [lines];
  const fill = header ? 'EEF3F0' : shading;
...
function buildScheduleDocument({ schedule, timeSlots, teachers }) {
  ...
  const bodyRows = teachers.map((t) => {
    if (!t.active) {
      return new TableRow({
        children: [
          cell(t.name, { align: AlignmentType.LEFT, width: nameColWidth, shading: 'F3F4F6' }),
          ...timeSlots.map(() => cell('請假', { width: slotColWidth, shading: 'F3F4F6' })),
          cell('-', { width: totalColWidth, shading: 'F3F4F6' }),
          cell('-', { width: totalColWidth, shading: 'F3F4F6' }),
        ],
      });
    }
  ...
  if (teachers.length === 0) {
    bodyRows.push(
      new TableRow({
        children: [cell('尚未新增老師', { align: AlignmentType.LEFT, width: nameColWidth })],
      })
    );
  }
